...
data <- data_orig %>% 
  ...
  rename(comm_name_orig=species, m=natmort, ssb_raw=ssbraw, r_raw=rraw, 
         ...
         r_age_yr=agerec, ssb_scaled=ssbstand, r_life=rlife, r_annual=rannual) %>% 
...
stocks <- data %>% 
  ...
  summarize(nyr=sum(!is.na(ssb_scaled) & !is.na(r_annual))) %>% 
...
# Add missing redfish order/family
stocks$order[stocks$species=="Sebastes fasciatus and mentella"] <- "Scorpaeniformes"
stocks$family[stocks$species=="Sebastes fasciatus and mentella"] <- "Sebastidae"
