...
fit_sr_ar1 <- function(data, type="Ricker"){
  ...
  if(FALSE){
    dyn.unload(paste(tmbdir, dynlib("srmodel_ar1.cpp"),sep="/") )
    file.remove(paste(tmbdir, c("srmodel_ar1.o","srmodel_ar1.dll"), sep="/") )
    compile("srmodel_ar1.cpp")
  }
  
  # Load TMB code
  dyn.load(dynlib("srmodel_ar1"))
  ...
  # Initialize model
  model <- MakeADFun(data=input.data, parameters=params, DLL="srmodel_ar1")
