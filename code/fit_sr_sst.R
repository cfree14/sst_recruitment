...
fit_sr_sst <- function(data, type="Ricker"){
  ...
  if(FALSE){
    dyn.unload(paste(tmbdir, dynlib("srmodel_sst.cpp"),sep="/") )
    file.remove(paste(tmbdir, c("srmodel_sst.o","srmodel_sst.dll"), sep="/") )
    compile("srmodel_sst.cpp")
  }
  
  # Load TMB code
  dyn.load(dynlib("srmodel_sst"))
  ...
  # Initialize model
  model <- MakeADFun(data=input.data, parameters=params, random="theta", DLL="srmodel_sst")
