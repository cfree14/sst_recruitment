...
plot_thetas_group <- function(results){
  ...
  sapply(1:nrow(vals), function(x) lines(x=c(vals$est_lo[x], vals$est_hi[x]), y=c(x,x), lwd=0.6)) #, col=vals$lcolor[x]))
  ...
  # Add group labels
  text(x=-2.3, y=nrow(vals):1, pos=4, labels=vals$group, xpd=NA, cex=0.8)
